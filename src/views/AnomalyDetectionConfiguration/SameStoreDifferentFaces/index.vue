...
<template>
    ...
    >
        ...
        >
            ...
            <el-form-item label="同店异脸告警数值（大于等于）" prop="threshold">
                <el-input-number
                    v-model="addConfiguration.threshold"
                    placeholder="请输入同店异脸告警数值"
                    class="w-full"
                    :min="0"
                    :max="999999999"
                />
...
const router = useRouter(),
    state = reactive({
        ...
        addConfigurationDialogVisible: false, // 新增同店异脸配置弹窗
        statusLoading: false, // 启用状态loading
        tableData: [],
    }),
    addConfigurationRef = ref(null), // 新增同店异脸配置表单ref
    ...
    rules = reactive({
        ...
        threshold: [{ required: true, message: '请输入同店异脸告警数值', trigger: 'blur' }],
...
const getTableData = async (init) => {
    ...
    try {
        const {
            ...
            'get',
            `/api/v1/tobacco/signingMultiFacesConfig/list?${qs.stringify(params)}`
        )
