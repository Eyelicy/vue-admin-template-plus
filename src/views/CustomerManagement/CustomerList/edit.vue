        ...
        <div class="w-full px-16">
            ...
            <div class="grid grid-cols-4 gap-4 gap-y-8">
                ...
                <descriptions-item label="风险预警等级">
                    ...
                    <el-button type="danger" plain @click="handleResetLevel"
                        v-if="details?.customerAlertLevel?.name">重置</el-button>
                </descriptions-item>
                ...
                <descriptions-item label="店招名称">
                    <el-input v-model="details.shopSignName" class="w-[120px]" @change="handleChange" />
                </descriptions-item>
            ...
            <Table :data="state.readNameList" :show-page="false">
                ...
                <el-table-column prop="signingCount" label="概率">
                    ...
                    <template #default="{ row }">
                        概率
                        {{
                    `${isNaN(((row?.signingCount / details?.totalSigningCount) * 100)) ? 0 : ((row?.signingCount /
                        details?.totalSigningCount) * 100).toFixed(
                            2
                        )}%`
                }}
...
const handleChange = async () => {
    const { code } = await tobaccoApi('put', `/api/v1/tobacco/customer`, details)
    if (code === 200) {
        ElMessage.success('修改成功')
    } else {
        ElMessage.error('修改失败')
